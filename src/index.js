...
if (process.env.SCHEDULE_MESSAGE)
{
	...
	var j = schedule.scheduleJob({hour: time[0], minute: time[1], dayOfWeek: 1}, async function(){
		const url = `https://i0.wp.com/eatery.se/wp-content/uploads/${moment().format("YYYY/MM")}/kista-nod-lunch-v${moment().week()}.png`;
		try {
...
if (process.env.ALLOW_COMMAND)
{
	client.on("message", async message => {
		...
		if (command === process.env.MENU_COMMAND) {
			...
			const date = moment().day(moment().format("DD")).week(args);
			log("LOG", `${author.username} requested the menu from week ${date.week()}.`);
			const url = `https://i0.wp.com/eatery.se/wp-content/uploads/${date.format("YYYY/MM")}/kista-nod-lunch-v${date.week()}.png`;
  			try {
